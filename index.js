...
plugin.register = async function (server, opts) {
  ...
  server.ext('onPostAuth', async function (req, h) {
    try {
      ...
      const { routeOptions, pluginOptions, combinedOptions } = options(
        req,
        opts
      );
      if (
        (pluginOptions.policy === 'deny' && combinedOptions.secure) ||
        (pluginOptions.policy === 'allow' && routeOptions.secure)
      ) {
        debug('policy if statement has evaluated to true');
        const handlerResult = combinedOptions.handler(req);
        const handlerObject = handlerResult.then
          ? await handlerResult
          : handlerResult;
        debug('handlerObject:');
        debug(handlerObject);
        const isAuthorized = await authorization.determineAuthorization(
          combinedOptions,
          handlerObject,
          req
        );
        ...
        if (!isAuthorized) {
          const forbiddenResult = combinedOptions.forbiddenPageFunction
            ? combinedOptions.forbiddenPageFunction(handlerObject, req, h)
            : Boom.forbidden();
          result =
            forbiddenResult && forbiddenResult.then
              ? await forbiddenResult
              : forbiddenResult;
        } else {
          result = h.continue;
        }
