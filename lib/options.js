...
module.exports = function (req, options) {
  const routeOptions =
    req.route.settings.plugins['hapi-acl-auth'] ||
    req.route.settings.plugins['hapiAclAuth'] ||
    {};
  ...
  debug(combinedOptions);
  const _options = optionsSchema.validate(combinedOptions);
  if (_options.error) {
    throw new Error(
      util.format('options could not be validated: %s', _options.error.message)
    );
  }
